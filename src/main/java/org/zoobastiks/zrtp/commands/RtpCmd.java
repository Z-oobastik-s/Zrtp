...
public class RtpCmd implements CommandExecutor, TabCompleter {
        ...
        switch (args[0].toLowerCase()) {
            case "reload":
                ...
                if (sender instanceof Player && !sender.hasPermission("rtp.reload")) {
                    plugin.getLang().sendMessage((Player) sender, Lang.Keys.NO_PERMISSION);
                    return true;
                }
                ...
                if (sender instanceof Player) {
                    plugin.getLang().sendMessage((Player) sender, Lang.Keys.RELOAD_SUCCESS);
                } else {
            ...
            case "help":
                ...
                if (sender instanceof Player) {
                    plugin.getLang().sendMessage((Player) sender, Lang.Keys.HELP_MESSAGE);
                } else {
        ...
        plugin.getTaskManager().findRandomLocation(world).thenAccept(location -> {
            if (location != null) {
                ...
                Bukkit.getScheduler().runTask(plugin, () -> {
                    ...
                    plugin.getLang().sendMessage(player, Lang.Keys.TELEPORT_SUCCESS);
                    ...
                    if (sender != player && sender instanceof Player) {
                        ...
                        sender.sendMessage("Игрок " + player.getName() + " телепортирован в мир " + world.getName());
                    }
                });
            } else {
                ...
                if (sender instanceof Player) {
                    ...
                    senderPlayer.sendMessage(plugin.getLang().parseComponent(plugin.getLang().getMessage(Lang.Keys.PREFIX) + 
                                             plugin.getLang().gradient("Не удалось найти безопасное место в мире " + world.getName(), "#FF5555", "#FF0000")));
                } else {
                    sender.sendMessage("Не удалось найти безопасное место в мире " + world.getName());
                }
                
                if (player != sender) {
                    plugin.getLang().sendMessage(player, Lang.Keys.UNSAFE_LOCATION);
                }
            }
        }).exceptionally(ex -> {
    ...
    private void handleTeleport(Player player) {
        ...
        if (!player.hasPermission("rtp.use")) {
            plugin.getLang().sendMessage(player, Lang.Keys.NO_PERMISSION);
            return;
        }
