...
public class TaskMgr {
    ...
    public void startTeleport(Player player) {
        ...
        if (currentTime - lastTeleport < cooldownTime && !player.hasPermission("rtp.nocooldown")) {
            ...
            plugin.getLang().sendMessage(player, Lang.Keys.COOLDOWN, "time", String.valueOf(remaining));
            return;
        }
        ...
        if (price > 0 && plugin.getEconomyManager().isEnabled()) {
            ...
            if (!plugin.getEconomyManager().hasMoney(player, price)) {
                ...
                plugin.getLang().sendMessage(player, Lang.Keys.NOT_ENOUGH_MONEY, "price", formattedPrice);
                return;
            }
        }
        ...
        if (price > 0 && plugin.getEconomyManager().isEnabled()) {
            ...
            String formattedPrice = plugin.getEconomyManager().formatMoney(price);
            plugin.getLang().sendMessage(player, Lang.Keys.MONEY_WITHDRAWN, "price", formattedPrice);
        }
        ...
        plugin.getLang().sendMessage(player, Lang.Keys.TELEPORT_STARTED, "delay", String.valueOf(effectDelay));
            ...
            if (onlinePlayer != null && onlinePlayer.isOnline()) {
                ...
                Bukkit.getScheduler().runTask(plugin, () -> {
                    if (location != null) {
                        ...
                        plugin.getLang().sendMessage(onlinePlayer, Lang.Keys.TELEPORT_SUCCESS);
                        ...
                        plugin.getLang().sendMessage(onlinePlayer, Lang.Keys.UNSAFE_LOCATION);
                    }
                });
            }
        }).exceptionally(ex -> {
        ...
        if (refundAmount > 0 && plugin.getEconomyManager().isEnabled()) {
            ...
            if (player != null && player.isOnline()) {
                ...
                if (plugin.getEconomyManager().depositMoney(player, refundAmount)) {
                    ...
                    plugin.getLang().sendMessage(player, Lang.Keys.MONEY_REFUNDED, "price", formattedPrice);
                }
            }
        }
